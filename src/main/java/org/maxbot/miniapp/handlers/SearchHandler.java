                            ...
                            .then(Mono.defer(() -> {
                                ...
                                return Flux.fromIterable(searchResponse.getHits())
                                        .flatMapSequential(hit -> {
                                            ...
                                            BotResponse singlePatentMessage = BotResponse.builder()
                                                    .notify(false)
                                                    .text(cardText)
                                        ...
                                        .then(); // Компонуем поток отправки в один завершающий сигнал
                            }))
                            .then(Mono.defer(() -> {
                                // 3. СБОРКА И ОТПРАВКА НИЖНЕГО МЕНЮ НАВИГАЦИИ (ПОСЛЕ ВСЕХ ПАТЕНТОВ)
